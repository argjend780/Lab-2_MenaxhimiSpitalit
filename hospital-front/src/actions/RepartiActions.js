...
export const getRepartis = (qytetiId, spitaliId) => async (dispatch) => {
    try {
        const response = await axios.get(`http://localhost:8081/repartet/list/${qytetiId}/${spitaliId}`);
        dispatch({
            type: GET_REPARTS,
...
export const createRepart = (qytetiId, spitaliId, reparti) => async (dispatch) => {
    try {
        const response = await axios.post(`http://localhost:8081/repartet/${qytetiId}/${spitaliId}`,reparti,{
           /* headers: {
...
export const getReparti = (qytetiId, spitaliId, repartiId) => async (dispatch) => {
    try {
        const response = await axios.get(`http://localhost:8081/repartet/get/${qytetiId}/${spitaliId}/${repartiId}`);
        dispatch({
            type: GET_REPARTI,
            payload: response.data
        });
...
export const deleteReparti = (qytetiId, spitaliId,repartiId) => async (dispatch) => {
    try {
        console.log(`Request URL: http://localhost:8081/repartet/delete/${qytetiId}/${spitaliId}/${repartiId}`); // Log request URL
        const response = await axios.delete(`http://localhost:8081/repartet/delete/${qytetiId}/${spitaliId}/${repartiId}`);
        dispatch({
            type: DELETE_REPARTI,
            payload: response.data
        });
